...
Detailed instructions for using the try server:
	...
	Note that --disable-tests is required because of bug 531680.

	Clone http://hg.mozilla.org/releases/mozilla-1.9.1/
	Update to the tag FIREFOX_3_1b3_RELEASE .
	Commit the patch generated by make-patch.sh.
	Commit, on top of that, a patch to change the default in
		configure.in to --disable-tests.
	On top of that, transplant the changesets (from the same
		repository): 712b74ad6d37 d08f35b964dd 0b981d1c3254
		09522dd1776e to avoid Windows build errors related to
		the source server.
