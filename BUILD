...
For each platform:
	...
	In a source tree from the Firefox 4.0 beta 11 release, extract
...
when run from the leak-monitor directory.  (Note that doing this doesn't
give you XPIs; you need to zip them manually from the extensions/
subdirectory of the archive.)

Detailed instructions for using the try server:
	Note that push-to-try is required because of bug 421347.

	Clone http://hg.mozilla.org/mozilla-central/
	Update to the tag FIREFOX_4_0b11_RELEASE or a near ancestor
		of it, such as 1c2d53a2dcfb.
	Commit the patch generated by make-patch.sh.
	Push to the try repository.

	At the end, need to manually restore leakmonitor.xpt and its
	interfaces line, since the packaging script via try doesn't
